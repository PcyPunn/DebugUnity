...
public class PigmomSprite : MonoBehaviour
{
    public static PigmomSprite Instance;
    ...
    void Start()
    {
        Debug.Log("PigMom script started.");

        if (targetSpriteRenderer != null)
        {
            Debug.Log("PigMom SpriteRenderer is assigned.");
            if (StaticClassForPassingData.PigMomTexture != null)
            {
                Debug.Log("Captured texture is not null.");
                // หมุนภาพก่อน
                Texture2D rotatedTexture = RotateTexture(StaticClassForPassingData.PigMomTexture, 270);
        ...
        {
            Debug.LogError("PigMom SpriteRenderer is not assigned.");
        }
    }
    ...
    private void SaveTexture(Texture2D texture)
    {
        ...
        PlayerPrefs.SetString("SavedPigMom", encodedTexture);
        PlayerPrefs.Save();
    ...
    {
        string encodedTexture = PlayerPrefs.GetString("SavedPigMom", null);
