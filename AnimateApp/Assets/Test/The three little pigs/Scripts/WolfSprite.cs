...
public class WolfSprite : MonoBehaviour
{
    public static WolfSprite Instance;
    ...
    void Start()
    {
        Debug.Log("Wolf script started.");

        if (targetSpriteRenderer != null)
        {
            Debug.Log("Wolf SpriteRenderer is assigned.");
            if (StaticClassForPassingData.WolfTexture != null)
            {
                Debug.Log("Captured texture is not null.");
                // หมุนภาพก่อน
                Texture2D rotatedTexture = RotateTexture(StaticClassForPassingData.WolfTexture, 270);
        ...
        {
            Debug.LogError("Wolf SpriteRenderer is not assigned.");
        }
    }
    ...
    private void SaveTexture(Texture2D texture)
    {
        ...
        PlayerPrefs.SetString("SavedWolf", encodedTexture);
        PlayerPrefs.Save();
    ...
    {
        string encodedTexture = PlayerPrefs.GetString("SavedWolf", null);
