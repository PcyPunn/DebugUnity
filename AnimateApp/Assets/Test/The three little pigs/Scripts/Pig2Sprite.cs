...
public class Pig2Sprite : MonoBehaviour
{
    public static Pig2Sprite Instance;
    ...
    void Start()
    {
        Debug.Log("Pig2 script started.");

        if (targetSpriteRenderer != null)
        {
            Debug.Log("Pig2 SpriteRenderer is assigned.");
            if (StaticClassForPassingData.Pig2Texture != null)
            {
                Debug.Log("Captured texture is not null.");
                // หมุนภาพก่อน
                Texture2D rotatedTexture = RotateTexture(StaticClassForPassingData.Pig2Texture, 270);
        ...
        {
            Debug.LogError("Pig2 SpriteRenderer is not assigned.");
        }
    }
    ...
    private void SaveTexture(Texture2D texture)
    {
        ...
        PlayerPrefs.SetString("SavedPig2", encodedTexture);
        PlayerPrefs.Save();
    ...
    {
        string encodedTexture = PlayerPrefs.GetString("SavedPig2", null);
