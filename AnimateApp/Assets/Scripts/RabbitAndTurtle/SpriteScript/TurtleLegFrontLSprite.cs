...
public class TurtleLegFrontLSprite : MonoBehaviour
{
    public static TurtleLegFrontLSprite Instance;
    ...
    void Start()
    {
        Debug.Log("TurtleLegFrontL script started.");

        if (targetSpriteRenderer != null)
        {
            Debug.Log("TurtleLegFrontL SpriteRenderer is assigned.");
            if (StaticClassForPassingData.TurtleTexture != null)
            {
                Debug.Log("Captured texture is not null.");
                // หมุนภาพก่อน
                Texture2D rotatedTexture = RotateTexture(StaticClassForPassingData.TurtleTexture, 270);
        ...
        {
            Debug.LogError("TurtleLegFrontL SpriteRenderer is not assigned.");
        }
    }
    ...
    private void SaveTexture(Texture2D texture)
    {
        ...
        PlayerPrefs.SetString("SavedTurtleRnT", encodedTexture);
        PlayerPrefs.Save();
    ...
    {
        string encodedTexture = PlayerPrefs.GetString("SavedTurtleRnT", null);
