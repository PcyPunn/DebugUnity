...
public class RabbitArmRightSprite : MonoBehaviour
{
    public static RabbitArmRightSprite Instance;
    ...
    void Start()
    {
        Debug.Log("RabbitArmRight script started.");

        if (targetSpriteRenderer != null)
        {
            Debug.Log("RabbitArmRight SpriteRenderer is assigned.");
            if (StaticClassForPassingData.RabbitTexture != null)
            {
                Debug.Log("Captured texture is not null.");
                // หมุนภาพก่อน
                Texture2D rotatedTexture = RotateTexture(StaticClassForPassingData.RabbitTexture, 270);
        ...
        {
            Debug.LogError("RabbitArmRight SpriteRenderer is not assigned.");
        }
    }
