...
public class RabbitSleepSprite : MonoBehaviour
{
    public static RabbitSleepSprite Instance;
    ...
    void Start()
    {
        Debug.Log("RabbitSleep script started.");

        if (targetSpriteRenderer != null)
        {
            Debug.Log("RabbitSleep SpriteRenderer is assigned.");
            if (StaticClassForPassingData.RabbitSleepTexture != null)
            {
                Debug.Log("Captured texture is not null.");
                // หมุนภาพก่อน
                Texture2D rotatedTexture = RotateTexture(StaticClassForPassingData.RabbitSleepTexture, 270);
        ...
        {
            Debug.LogError("RabbitSleep SpriteRenderer is not assigned.");
        }
    }
    ...
    private void SaveTexture(Texture2D texture)
    {
        ...
        PlayerPrefs.SetString("SavedRabbitSleepRnT", encodedTexture);
        PlayerPrefs.Save();
    ...
    {
        string encodedTexture = PlayerPrefs.GetString("SavedRabbitSleepRnT", null);
