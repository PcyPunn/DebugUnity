...
public class RabbitWorrySprite : MonoBehaviour
{
    public static RabbitWorrySprite Instance;
    ...
    void Start()
    {
        Debug.Log("RabbitWorry script started.");

        if (targetSpriteRenderer != null)
        {
            Debug.Log("RabbitWorry SpriteRenderer is assigned.");
            if (StaticClassForPassingData.RabbitWorryTexture != null)
            {
                Debug.Log("Captured texture is not null.");
                // หมุนภาพก่อน
                Texture2D rotatedTexture = RotateTexture(StaticClassForPassingData.RabbitWorryTexture, 270);
        ...
        {
            Debug.LogError("RabbitWorry SpriteRenderer is not assigned.");
        }
    }
    ...
    private void SaveTexture(Texture2D texture)
    {
        ...
        PlayerPrefs.SetString("SavedRabbitWorryRnT", encodedTexture);
        PlayerPrefs.Save();
    ...
    {
        string encodedTexture = PlayerPrefs.GetString("SavedRabbitWorryRnT", null);
