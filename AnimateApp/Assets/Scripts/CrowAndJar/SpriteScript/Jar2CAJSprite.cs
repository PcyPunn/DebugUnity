...
public class Jar2CAJSprite : MonoBehaviour
{
    public static Jar2CAJSprite Instance;
    ...
    void Start()
    {
        Debug.Log("Jar2 script started.");

        if (targetSpriteRenderer != null)
        {
            Debug.Log("Jar2 SpriteRenderer is assigned.");
            if (StaticClassForPassingData.JarCAJTexture != null)
            {
                Debug.Log("Captured texture is not null.");
                // หมุนภาพก่อน
                Texture2D rotatedTexture = RotateTexture(StaticClassForPassingData.JarCAJTexture, 270);
        ...
        {
            Debug.LogError("Jar2 SpriteRenderer is not assigned.");
        }
    }
    ...
    private void SaveTexture(Texture2D texture)
    {
        ...
        PlayerPrefs.SetString("SavedJar2CAJ", encodedTexture);
        PlayerPrefs.Save();
    ...
    {
        string encodedTexture = PlayerPrefs.GetString("SavedJarImage", null);
