...
public class CrowWalkWingCAJSprite : MonoBehaviour
{
    public static CrowWalkWingCAJSprite Instance;
    ...
    void Start()
    {
        Debug.Log("CrowWalkWing script started.");

        if (targetSpriteRenderer != null)
        {
            Debug.Log("CrowWalkWing SpriteRenderer is assigned.");
            if (StaticClassForPassingData.CrowWalkCAJTexture != null)
            {
                Debug.Log("Captured texture is not null.");
                // หมุนภาพก่อน
                Texture2D rotatedTexture = RotateTexture(StaticClassForPassingData.CrowWalkCAJTexture, 270);
        ...
        {
            Debug.LogError("CrowWalkWing SpriteRenderer is not assigned.");
        }
    }
    ...
    private void SaveTexture(Texture2D texture)
    {
        ...
        PlayerPrefs.SetString("SavedCrowWalkWingCAJ", encodedTexture);
        PlayerPrefs.Save();
    ...
    {
        string encodedTexture = PlayerPrefs.GetString("SavedCrowWalkImage", null);
