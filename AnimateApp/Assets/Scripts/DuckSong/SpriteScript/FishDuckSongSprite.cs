...
public class FishDuckSongSprite : MonoBehaviour
{
    public static FishDuckSongSprite Instance;
    ...
    void Start()
    {
        Debug.Log("Fish script started.");

        if (targetSpriteRenderer != null)
        {
            Debug.Log("Fish SpriteRenderer is assigned.");
            if (StaticClassForPassingData.FishTexture != null)
            {
                Debug.Log("Captured texture is not null.");
                // หมุนภาพก่อน
                Texture2D rotatedTexture = RotateTexture(StaticClassForPassingData.FishTexture, 270);
        ...
        {
            Debug.LogError("Fish SpriteRenderer is not assigned.");
        }
    }
    ...
    private void SaveTexture(Texture2D texture)
    {
        ...
        PlayerPrefs.SetString("SavedFishDuckSong", encodedTexture);
        PlayerPrefs.Save();
    ...
    {
        string encodedTexture = PlayerPrefs.GetString("SavedFishDuckSong", null);
