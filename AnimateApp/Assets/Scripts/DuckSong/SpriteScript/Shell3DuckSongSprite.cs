...
public class Shell3DuckSongSprite : MonoBehaviour
{
    public static Shell3DuckSongSprite Instance;
    ...
    void Start()
    {
        Debug.Log("Shell3 script started.");

        if (targetSpriteRenderer != null)
        {
            Debug.Log("Shell3 SpriteRenderer is assigned.");
            if (StaticClassForPassingData.ShellTexture != null)
            {
                Debug.Log("Captured texture is not null.");
                // หมุนภาพก่อน
                Texture2D rotatedTexture = RotateTexture(StaticClassForPassingData.ShellTexture, 270);
        ...
        {
            Debug.LogError("Shell3 SpriteRenderer is not assigned.");
        }
    }
    ...
    private void SaveTexture(Texture2D texture)
    {
        ...
        PlayerPrefs.SetString("SavedShellDuckSong", encodedTexture);
        PlayerPrefs.Save();
    ...
    {
        string encodedTexture = PlayerPrefs.GetString("SavedShellDuckSong", null);
